    ...
    <div class="container mx-auto px-6 lg:px-12">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            ...
            <article class="bg-white p-10 md:p-12 shadow-2xl rounded-3xl border border-gray-200 transform transition-all duration-500 hover:scale-[1.02]">
                ...
                <!-- Success & Errors -->
                @if(session('success'))
                <div class="mb-6 p-4 bg-green-100 text-green-800 rounded-lg border border-green-200 shadow-sm animate-fadeIn">
                    {{ session('success') }}
                </div>
                @endif

                @if($errors->any())
                <div class="mb-6 p-4 bg-red-100 text-red-800 rounded-lg border border-red-200 shadow-sm animate-fadeIn">
                ...
                <form action="{{ route('booking.submit') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="phone" id="phone" placeholder="+256 77 123 4567" value="{{ old('phone') }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="event_type" id="event_type" placeholder="Wedding, Birthday, Corporate..." value="{{ old('event_type') }}" required
                    ...
                    <div>
                        <label for="message" class="block text-sm font-semibold text-gray-700 mb-2">Additional Details (Optional)</label>
                        <textarea name="message" id="message" placeholder="Any additional info..."
                            class="w-full px-6 py-4 border border-gray-300 rounded-xl shadow-inner focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none h-40 hover:shadow-lg transition duration-300">{{ old('message') }}</textarea>
                    </div>
            ...
            <aside class="bg-gradient-to-br from-purple-600 to-indigo-700 p-10 rounded-3xl shadow-2xl text-white flex flex-col justify-center space-y-8">
                ...
                <p class="text-purple-100">If you have questions about bookings, reach out via WhatsApp or Email.</p>
